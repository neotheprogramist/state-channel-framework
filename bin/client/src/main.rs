...
mod tests {
    use super::*;
    async fn create_aggrement(quantity:i64, price:i64,address: &String,url_request_quote:&String,url_request_settlement_proof:&String, client: &Client,url_accept_contract:&String)-> Result<(),  Box<dyn std::error::Error>> {
    ...
    async fn test_set_price() -> Result<(),  Box<dyn std::error::Error>>  {
        ...
        create_aggrement(quantity:1,price:63111,address:address,&url_request_quote, &url_request_settlement_proof,&client,&url_accept_contract).await?;
