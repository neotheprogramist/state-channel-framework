...
async fn main() -> Result<(), Box<dyn StdError>> {
    ...
    let state: AppState = AppState { db,mock_account:server_mock_account.clone()};
    ...
    prepare_and_save_data(path_in.to_string(),settlement_proof.clone(),client_mock_account.clone(),server_mock_account.clone()).await?;
    let path_out: &str = "bin/json_generator/output/out.json";
    save_out(path_out.to_string(), settlement_price,settlement_proof.diff).await?;
    Ok(())
